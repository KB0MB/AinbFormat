...
internal static class FixtureJson
{
    public static AinbDocument Read(string path)
    {
        ...
        foreach (string field in new[] { "Blackboard", "Expressions", "Unknown Section 0x58" })
            if (d.GetProperty(field).EnumerateObject().Any())
                throw new InvalidDataException($"Test translator does not implement {field}.");
        if (d.GetProperty("Replacement Table").GetArrayLength() != 0)
            throw new InvalidDataException("Replacement table in test fixture.");
        return new()
        {
    ...
    private static AinbNode Node(JsonElement n)
    {
        ...
        foreach (var flag in n.GetProperty("Flags").EnumerateArray())
            flags |= flag.GetString() switch
            {
    ...
    private static AinbParameterFlags Flags(JsonElement p)
    {
        ...
        foreach (var flag in p.GetProperty("Flags").EnumerateArray())
            flags |= flag.GetString() switch
            {
    ...
    private static void CheckKeys(JsonElement d, params string[] keys)
    {
        foreach (var p in d.EnumerateObject())
            if (!keys.Contains(p.Name, StringComparer.Ordinal))
                throw new InvalidDataException($"Unrepresented test field: {p.Name}");
    }
}
